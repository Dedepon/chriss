...
export class OrderService {
  ...
  public loadOrders(): Observable<Order[]> {
    return this.http.get<Order[]>(ORDER_LIST_URL).pipe(tap((orders: Order[]) => this.ordersSubject$.next(orders)));
  }
  ...
  public closeOrder(order: Order): Observable<Order> {
    return this.http.put<Order>(ORDER_LIST_URL + "/" + order._id + "/close", {});
  }
